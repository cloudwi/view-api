...
PRODUCTION_VIEWS = [
  # 음식/맛집 (15개)
  { title: "짜장면 vs 짬뽕", options: %w[짜장면 짬뽕 짬짜면], category: "food" },
  { title: "탕수육 부먹 vs 찍먹", options: %w[부먹 찍먹 반반], category: "food" },
  { title: "최고의 치킨 메뉴는?", options: %w[후라이드 양념 간장 마늘 반반], category: "food" },
  { title: "라면에 밥 말아먹기", options: [ "당연히 한다", "절대 안한다", "가끔" ], category: "food" },
  { title: "삼겹살 vs 목살 vs 항정살", options: %w[삼겹살 목살 항정살 갈비], category: "food" },
  { title: "떡볶이 맵기는?", options: %w[순한맛 중간맛 매운맛 불닭맛], category: "food" },
  { title: "초밥 vs 회", options: [ "초밥", "회", "둘 다 좋아" ], category: "food" },
  { title: "아침 메뉴 추천", options: %w[토스트 김밥 죽 샌드위치 굶는다], category: "food" },
  { title: "커피는 어떻게?", options: [ "아메리카노", "라떼", "바닐라라떼", "안 마심" ], category: "food" },
  { title: "여름 디저트 최애는?", options: %w[빙수 아이스크림 아이스커피 빙과류], category: "food" },
  { title: "야식 메뉴 BEST", options: %w[치킨 피자 족발 라면 떡볶이], category: "food" },
  { title: "햄버거 프랜차이즈 픽", options: %w[맥도날드 버거킹 롯데리아 맘스터치 KFC], category: "food" },
  ...
  { title: "좋아하는 계절은?", options: %w[봄 여름 가을 겨울], category: "daily" },
  ...
  { title: "숙소는 어디로?", options: %w[호텔 에어비앤비 펜션 게스트하우스], category: "travel" },
  { title: "패키지 vs 자유여행", options: %w[패키지 자유여행 반반], category: "travel" },
  { title: "여행 사진 많이 찍으시나요?", options: [ "엄청 많이", "적당히", "거의 안 찍음" ], category: "travel" },
  { title: "비행기 좌석 선호도", options: %w[창가 복도 상관없음], category: "travel" },
  ...
  { title: "패딩 브랜드 추천", options: %w[노스페이스 디스커버리 네파 유니클로 기타], category: "fashion" },
  ...
  { title: "좋아하는 영화 장르", options: %w[액션 로맨스 코미디 스릴러 SF 공포], category: "hobby" },
  ...
  { title: "커피챗 어떻게 생각하세요?", options: %w[좋다 부담스럽다 상관없음], category: "work" },
  ...
  { title: "소개팅 vs 클럽/나이트", options: [ "소개팅", "클럽/나이트", "앱", "자연스럽게" ], category: "adult" },
  { title: "술자리 적정 인원", options: [ "2명(둘이서)", "4-6명", "8명 이상", "혼술" ], category: "adult" },
...
CATEGORIES.each do |cat_data|
  ...
  puts "  - #{category.icon} #{category.name} (#{category.slug})"
end
puts "✅ #{Category.count}개 카테고리 생성 완료\n"
...
created_views.each do |view|
  ...
  voters.each do |voter|
    ...
    begin
      Vote.create!(user: voter, view_option: option)
      vote_count += 1
...
created_views.each do |view|
  rand(1..12).times do
    ...
    Comment.create!(user: commenter, view: view, content: content)
    comment_count += 1
  end
end
...
Category.ordered.each do |cat|
  puts "  - #{cat.icon} #{cat.name}: #{cat.views.count}개"
end
puts "=" * 50
puts "\n💡 테스트 계정:"
